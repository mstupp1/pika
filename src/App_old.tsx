  ...
  useEffect(() => {
      ...
      setBerryPositions((prev) => {
        ...
        const updated = prev.map((berry) => {
          ...
          if (newY <= 0.5) {
            ...
            return {
              ...berry,
              position: [berry.position[0], 0.5, berry.position[2]] as [
                number,
                number,
                number
              ],
          ...
          return {
            ...berry,
            position: [berry.position[0], newY, berry.position[2]] as [
              number,
              number,
              number
            ],
    ...
    const spawnInterval = setInterval(() => {
      ...
      setBerryPositions((prev) => {
        ...
        const numRainBerries =
          baseRainCount + progressBonus + Math.floor(Math.random() * 5); // 5-25 berries per rain
    ...
    (isGolden: boolean, isPurple: boolean) => {
      ...
      setBerryPositions((prev) => {
        ...
        const baseSpawn = Math.min(
          15,
          Math.max(8, Math.floor(targetCount - currentCount + 6))
        );
